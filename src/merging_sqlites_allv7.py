...
class SqliteDBMerged(object):
    ...
    def merge(self):
        ...
        ccc.execute("CREATE TABLE weights (gene text NOT NULL, rsid text NOT NULL, varID text NOT NULL, ref text NULL, alt text NULL, beta real NULL, tissue text NOT NULL, PRIMARY KEY(gene, rsid, tissue))")
        ...
        tableName = ['construction', 'model_summaries', 'sample_info', 'weights']
        ...
        for i in range(len(database_names)):
            ...
            tissue_name = database_names[i].split('.')[0][:-22]
            tissue_name = tissue_name.split('/')[len(tissue_name.split('/'))-1][8:]
            print(tissue_name)
        ...
        merged_model_summaries = glob.glob(self.output_path + '*model_summaries.csv')
        ...
        for list in [merged_model_summaries, merged_construction, merged_weights, merged_sample_info]:
            merged_final = ''
            merged = []
            ...
            if 'model_summaries' in list[0]:
                merged_final.to_csv(self.output_path + self.merged_db_name.split('.')[0] + "_" + 'model_summaries_final.csv', index=None)
